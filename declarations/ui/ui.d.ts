...
declare namespace WoWAPI {
  ...
  interface ObjectHookScript<T extends UIObject> {
    ...
    HookScript(
      event: Event.OnAny,
      handler: (frame: T, ...args: any[]) => void
    ): void;
    HookScript(
      event: 'OnClick',
      handler: (frame: T, button: MouseButton, down: boolean) => void
    ): void;
    HookScript(
      event: 'OnEnter' | 'OnLeave',
      handler: (frame: T, motion: Unknown) => void
    ): void;
    HookScript(
      event: 'OnEvent',
      handler: (
        frame: T,
        eventName: Event.OnAny & Event,
        ...args: any[]
      ) => void
    ): void;
    HookScript(
      event: 'OnHide' | 'OnShow' | 'OnLoad',
      handler: (frame: T) => void
    ): void;
    HookScript(
      event: 'OnMouseDown' | 'OnMouseUp',
      handler: (frame: T, button: MouseButton) => void
    ): void;
    HookScript(
      event: 'OnMouseWheel',
      handler: (frame: T, delta: MouseWheelDelta) => void
    ): void;
    HookScript(
      event: 'OnUpdate',
      handler: (frame: T, elapsed: number) => void
    ): void;
    HookScript(
      event: 'OnValueChanged',
      handler: (frame: T, changed: any) => void
    ): void;
    HookScript(
      event: 'OnTextChanged',
      handler: (frame: T, text: string) => void
    ): void;
    HookScript(
      event: Event.OnAny,
      handler?: (frame: T, ...args: any[]) => void
  ...
  interface ObjectSetScript<T extends UIObject> {
    ...
    SetScript(
      event: 'OnTextChanged',
      handler: (frame: T, isUserInput: boolean) => void
    ): void;
    SetScript(
      event: Event.OnAny,
      handler?: (frame: T, ...args: any[]) => void
  ...
  interface Frame
    extends Region,
      ObjectHookScript<Frame>,
      ObjectSetScript<Frame> {
    /**
 ...
 * @param width the new width
 */
declare function UIDropDownMenu_SetWidth(
  dropdown: WoWAPI.Frame,
  width: number
): void;
 ...
 * @param text the text to set
 */
declare function UIDropDownMenu_SetText(
  dropdown: WoWAPI.Frame,
  text: string
): void;
 ...
 */
declare function UIDropDownMenu_Initialize(
  dropdown: WoWAPI.Frame,
  callback: (self: WoWAPI.Frame, level: number, menuList: number) => void
): void;
 ...
 */
declare function InterfaceOptions_AddCategory(
  panel: WoWAPI.FrameInterfaceCategory
): void;
